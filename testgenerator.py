...
class Kasumi:
    ...
    def fun_FO(self, input, round_i):
        ...
        #print 'out FO', (out_left << 16) | out_right
    ...
    def fun_FL(self, input, round_i):
        ...
        # assert _bitlen(out_left)  <= 16
        # assert _bitlen(out_right) <= 16
        #print 'out FL', (out_left << 16) | out_right
...
if __name__ == '__main__':
    ...
    for j in range(0,5000):
        ...
        for i in range(0,16):
            x = rand.randint(0,15)
            plain = plain + hex(x)[2:]
        plains.append(hex(int(plain[1:], 16)))
        if (j < 4999):
            print(plain.upper() + "\",", file=f)
        else:
            print(plain.upper() + "\"];", file=f)
    ...
    for j in range(0,5000):
        ...
        for i in range(0,32):
            x = rand.randint(0,15)
            key = key + hex(x)[2:]
        keys.append(hex(int(key[1:], 16)))
        if (j < 4999):
            print(key.upper() + "\",", file=k)
        else:
            print(key.upper() + "\"];", file=k)

    k.close()

    c = open("cyphertests.txt", 'w+')
    print("C:=[", file=c)
    for j in range(0,5000):
        my_kasumi = Kasumi()
        my_kasumi.set_key(int(keys[j], 16))
        encrypted = my_kasumi.enc(int(plains[j], 16))
        string = (hex(encrypted))[2:]
        while len(string) < 16:
            string = "0" + string
        if (j < 4999):
        	string = "\"" + string + "\","
